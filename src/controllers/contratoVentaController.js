...
function addMonthsPreserveDay(date, months) {
  ...
  // establecer día deseado o último del mes
...
// ======================================================
// Helper: normalizar teléfono (simple)
// ======================================================
...
exports.createContrato = asyncHandler(async (req, res) => {
  ...
  if (parseFloat(enganche) >= parseFloat(precio_total)) {
    return res.status(400).json({
      message: 'El enganche debe ser menor que el precio total',
    });
  }
  ...
  try {
    ...
    const loteRes = await client.query(
      'SELECT id_propiedad, estado_propiedad FROM lote WHERE id_propiedad=$1 FOR UPDATE',
      [id_lote]
    );
    ...
    if (lote.estado_propiedad !== 'disponible') {
      await client.query('ROLLBACK');
      return res.status(400).json({
        message: `Lote no disponible (estado actual: ${lote.estado_propiedad})`,
      });
    }

    // Cliente: verificar o crear
    let finalClienteId = id_cliente ? parseInt(id_cliente) : null;

    if (finalClienteId) {
      const clienteExist = await client.query(
    ...
    const mensualidad = parseFloat(
      ((precioNum - engancheNum) / plazoNum).toFixed(2)
    );

    // Generar pagos mismo día
    const pagos = [];
    ...
    for (let i = 1; i <= plazoNum; i++) {
      ...
      pagos.push({
        ...
        numero_pago: i,
        monto: mensualidad,
        fecha_pago: fechaPago.toISOString().split('T')[0], // formato YYYY-MM-DD
    ...
    await client.query(
      'UPDATE lote SET estado_propiedad=$1 WHERE id_propiedad=$2',
      ['en proceso', id_lote]
    );
